...
- native histogram aggregation and bound edge cases, including `sum by (...)`
  ...
  arithmetic and non-bool `==`/`!=` comparisons for both classic/custom and
  exponential native histograms, invalid scalar/histogram and
  histogram/histogram drop shapes, and `histogram_fraction(-Inf, Inf, ...)`,
...
histogram binary operator edge cases such as histogram `bool` comparisons, set
operators, and mixed classic/exponential operands, native histogram error/drop
cases beyond custom bucket coarsening, and deeper OTLP
