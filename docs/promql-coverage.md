...
- native histogram aggregation and bound edge cases, including `sum by (...)`
  ...
  absence for `histogram_count`, and `histogram_fraction(-Inf, Inf, ...)`,
  plus `histogram_avg` dropping float-only input and preserving only the native
