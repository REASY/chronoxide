...
- OTLP typed Histogram projection to `_count`, `_sum`, and `_bucket`, including
  ...
  histograms, including direct native-function rate and stale-fragment handling
  over delta Histogram samples;
...
- native histogram aggregation and bound edge cases, including `sum by (...)`
  ...
  histogram sum/scalar arithmetic, finite-plus-infinite and same-infinite
  subtraction edges, and scaled aggregation for both custom and exponential
  histograms,
  `group_left` /
...
- query_range step output for stored selectors, offsets, label functions,
  ...
  composition, and binary/scalar composition;
