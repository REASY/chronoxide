...
- native histogram aggregation and bound edge cases, including `sum by (...)`
  ...
  and histogram/scalar `/` arithmetic, invalid scalar/histogram drop shapes,
  and `histogram_fraction(-Inf, Inf, ...)`, plus `histogram_avg` dropping
...
histogram/drop edge cases such as histogram/histogram arithmetic and
comparisons, native histogram error/drop cases beyond custom bucket coarsening,
and deeper OTLP
