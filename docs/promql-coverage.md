...
| Instant query API | Supported | Supported | Chronoxide exposes explicit evaluation-time core session methods, the `chronoxide-query` tool, and Prometheus-compatible GET/form-POST `/api/v1/query` through `chronoxide-api`. Results are PromQL-shaped vectors/scalars represented as segment query results. Whitefalcon exposes HTTP instant query endpoints. |
| Range query API | Partial | Supported with WF-specific timing | Chronoxide `query_promql_range` defaults to evaluating the instant expression independently at each step and merging by labelset; `chronoxide-api` exposes that production path through Prometheus-compatible GET/form-POST `/api/v1/query_range`. A sealed-store session and `chronoxide-query` can explicitly select the non-production `one-pass-assume-scalar` diagnostic comparator for unlimited `sum by (...)` / `count by (...)` over `rate()` of one exact direct metric. Unsupported shapes and finite limits fall back before experimental I/O; observing typed Histogram, ExponentialHistogram, or Summary chunks after the union decode is a terminal error rather than an omission or retry. Focused tests compare Float/Int64 results and semantic fingerprints with repeated execution, cover fallback/cache behavior and typed-source errors, and the independent decoded-chunk readback oracle now checks bounded multi-step scalar `rate()` without calling production evaluation helpers. Golden coverage also includes stored samples, scalar/rate steps, offsets, label functions, binary/scalar and nested vector-vector composition, classic/OTLP/native histogram projections, native custom/exponential histogram fraction/avg, native histogram scalar float-drop behavior, `changes`/`resets`, stale-latest absence composition, and sealed-plus-head float and typed-histogram range cases. More parity tests are still needed for subquery and deeper native-histogram range composition. Whitefalcon shifts range results by granularity because its storage is look-ahead. |
| Vector selectors | Supported | Partial | Chronoxide supports metric shorthand, brace-only selectors, equality, inequality, positive regex, negative regex, metric-name regex, and OTLP name normalization. As in Prometheus, every matcher evaluates an absent label as `""`: equality and regex predicates that match empty include absent labels, while their negations exclude them. Schema-6 sealed candidates are checked against their complete stored labels after v7 index planning, so a false-positive posting cannot become a returned series. Empty-matching predicates use transient 256-entry superset-scan batches and do not admit scanned misses to the persistent full-entry cache. Focused coverage includes explicit-empty, absent, and non-empty labels, mixed selective equality, native Histogram selection, and an in-range v7 posting-ref substitution; the Prometheus golden suite independently covers equality-to-empty. Whitefalcon selectors are tied to its label/grouping model and warn when grouping is implicit. |
...
| Common range functions | Partial | Partial | Chronoxide supports `rate`, `increase`, `delta`, `irate`, `idelta`, `changes`, `resets`, `last_over_time`, `count_over_time`, `present_over_time`, `sum_over_time`, `avg_over_time`, `stddev_over_time`, `stdvar_over_time`, `min_over_time`, `max_over_time`, `deriv`, `predict_linear`, `quantile_over_time`, and `double_exponential_smoothing` / `holt_winters`. Focused and Prometheus-golden coverage distinguishes the exact stale marker from ordinary `NaN`/`+Inf`/`-Inf` in scalar `rate`/`increase`; focused native Histogram/ExponentialHistogram coverage checks outcome-discriminating ordinary non-finite sum first/interior/final endpoints for direct native evaluation and virtual `_sum` projections, plus signed finite-negative delta sums across single/multi, native/virtual, rate/increase paths while count/bucket results remain valid. Direct native Histogram / ExponentialHistogram `changes()` and `resets()` are covered for observable component changes/decreases. Remaining gaps are subquery arguments, full histogram operand parity, and deeper composed non-finite cases. Whitefalcon implements a subset through bucketed aggregation; `deriv`, `predict_linear`, and `holt_winters` are lexer tokens but not implemented by the visitor. |
...
| Staleness | Partial | Divergent | Chronoxide persists Prometheus stale markers and omits only the exact marker from `rate()`/`increase()` input without turning it into an extrapolation boundary or reset. Golden coverage includes both `rate` and `increase` for scalar counters and delta Histogram/ExponentialHistogram virtual `_count`, `_sum`, and `_bucket` projections, plus direct native Histogram/ExponentialHistogram `rate()` across interior stale markers. Focused default-CI tests cover decreasing and equal/increasing post-stale delta fragments for both typed families, including an ExponentialHistogram fragment whose first post-gap value equals the pre-gap value before increasing. Golden coverage also includes binary arithmetic, `or`, and `unless` vector matching with stale operands; query_range aggregation over a stale step; stale latest custom/exponential native histogram absence; and stale custom/exponential native histogram vector matching. More stale marker parity tests are needed across deeper compositions and additional native histogram operand combinations. Whitefalcon filters NaNs from output, which differs from Prometheus. |
| Counter resets | Partial | Partial | Chronoxide handles counter decreases and OTLP reset hints for scalar and typed histogram rate/increase paths. Focused tests exercise an explicit cumulative `CounterReset` after an omitted stale sample through both direct native Histogram and ExponentialHistogram count/sum evaluation. Direct native Histogram / ExponentialHistogram `resets()` also counts observable component decreases. More temporality boundary tests remain. Whitefalcon has simpler cumulative/delta handling in its rate evaluator. |
| OTLP temporality | Partial | Not applicable | Chronoxide preserves OTLP temporality and projects delta histograms/exponential histograms to cumulative PromQL-shaped series. Golden coverage compares delta Histogram and ExponentialHistogram projections against equivalent cumulative Prometheus series, including both `rate` and `increase` across interior stale markers for `_count`, `_sum`, and `_bucket`, ordinary non-finite delta Histogram sums, direct native-function rate, reset-boundary projections, and query_range steps. A stale datapoint resets only the internal delta projection accumulator; the synthetic first post-stale fragment hint becomes unknown reset detection, so decreasing boundaries reset while equal/increasing boundaries do not, and the stale marker does not truncate the PromQL range. Stored cumulative reset hints remain authoritative. Every selected non-stale delta interval must have a present start strictly before its timestamp; focused single/multi H/ExH tests reject missing, equal, and future starts in direct and virtual rate/increase shapes while stale gaps remain exempt. Physical storage boundaries do not create resets for continuous adjacent intervals; focused Schema 7/8 tests cover cross-segment stitching, while same-fragment gaps and overlaps create logical boundaries for Histogram and ExponentialHistogram virtual counts and buckets. Virtual evaluation retains one aligned pre-range cumulative sample only as a subtraction seed, with focused signed H/ExH regressions proving that its interval is not imported. Direct native single intervals aggregate directly. For multi-sample delta ranges, native count/buckets use cumulative-shaped Prometheus counter math while virtual count/buckets aggregate valid intervals; optional signed IEEE sums aggregate intervals directly on both paths and never invalidate the count/bucket shape. Deeper reset and staleness compositions remain Chronoxide-specific correctness work. |
...
- instant/vector composition: `sum by`, `count by`, `avg by`, `min by`,
  ...
  `group_left`, `group_right`, and equality-to-empty selection across explicit
  empty and absent labels;
...
- stale and non-finite samples in selected aggregation, range, binary,
  vector-matching, and `count_values` paths, including scalar `rate`/`increase`
  distinction between exact stale and ordinary `NaN`/`+Inf`/`-Inf`, positive
  infinity aggregation/range propagation, mixed `+Inf`/`-Inf` aggregate NaN
  behavior, and Prometheus label spelling for both `+Inf` and `-Inf`;
...
- OTLP typed Histogram projection to `_count`, `_sum`, and `_bucket`, including
  cumulative projection from delta temporality and stale `rate`/`increase`
  coverage for all three virtual shapes, plus ordinary `NaN`/`+Inf`/`-Inf`
  and signed finite-negative delta-sum propagation without count/bucket loss,
  plus native typed Histogram
...
- OTLP typed ExponentialHistogram `_bucket` projection, including cumulative
  projection from delta temporality and stale `rate`/`increase` coverage for
  virtual `_count`, `_sum`, and `_bucket`, signed/non-finite delta sums, plus
  native `histogram_quantile` and
...
- native histogram aggregation and bound edge cases, including `sum by (...)`
  ...
  only the native histogram output from mixed float/native input for both
  custom and exponential native histograms;
...
- query_range step output for stored selectors, offsets, label functions,
  ...
  projection, direct native-function rate over OTLP delta
  Histogram/ExponentialHistogram stale fragments, and native
  custom/exponential histogram rate/aggregation quantiles, fraction/avg
  composition, custom and exponential scalar float-drop behavior,
