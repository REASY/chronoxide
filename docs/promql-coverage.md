...
- native histogram aggregation and bound edge cases, including `sum by (...)`
  ...
  classic/custom and exponential native histograms, same-kind `and` / `or` /
  `unless` set operators for both classic/custom and exponential native
  histograms, invalid scalar/histogram and histogram/histogram drop shapes, and
...
histogram binary operator edge cases such as mixed classic/exponential operands
including set operators, native histogram error/drop cases beyond custom bucket
