...
GATE_TOOL="$SCRIPT_DIR/query_label_storage_ab_gate.py"
...
DEFAULT_QUERY_MANIFEST="$SCRIPT_DIR/query_label_storage_queries.json"
...
Usage:
  RUN_NOTE='controlled run; host otherwise quiet' \\
    docs/experiments/storage_vnext/query_label_storage_ab_run.sh
...
Each query runs in a fresh process. OwnedStrings and SharedAtoms order alternates
by repetition; every process evaluates once cold and once warm in one session.
One copied release binary serves both policies. Label materialization is pinned
to full, chunk reads to pread, and every range scalar cache to zero bytes.

Footer validation and independent readbacks run before measurement. Timed
processes never validate footers. POSIX_FADV_DONTNEED plus fincore measure Linux
page-cache residency only; they do not flush device/controller caches.
...
die() {
    echo "query-label-storage A/B: $*" >&2
    exit 2
}

note() {
    echo "query-label-storage A/B: $*"
}
...
if [[ -z "${RESULT_DIR:-}" ]]; then
    ...
    RESULT_DIR="$RESULT_PARENT/query-label-storage-ab-$(date +%Y%m%d-%H%M%S)"
fi
...
FROZEN_GATE_TOOL="$METADATA_DIR/query_label_storage_ab_gate.py"
...
for required_help in \
    '--storage-layout' '--label-materialization' '--query-label-storage' \
    '--range-scalar-cache-max-bytes' '--verify-readbacks' \
    '--validate-segment-footers' 'schema8' 'owned-strings' 'shared-atoms'; do
...
for harness_file in \
    query_label_storage_ab_run.sh query_label_storage_ab_gate.py \
...
sha256sum \
    "$METADATA_DIR/query_label_storage_ab_run.sh" \
    "$METADATA_DIR/query_label_storage_ab_gate.py" \
...
policy_order_for_repetition() {
    if (( $1 % 2 == 1 )); then
        printf 'owned-strings shared-atoms\n'
    else
        printf 'shared-atoms owned-strings\n'
    fi
}

{
    printf 'process_label\tquery_name\tcategory\tmode\trepetition\torder_index\tquery_label_storage\tcorpus\n'
...

{
    ...
    stat -f -c 'corpus_filesystem_type=%T corpus_mount=%m' "$SCHEMA8_DIR" || true
...
run_validation_passes() {
    ...
    /usr/bin/time -v -o "$VALIDATION_DIR/readbacks.time.txt" \
        "$RUN_BIN" --segments-dir "$SCHEMA8_DIR" --storage-layout schema8 \
        --sample-limit-per-kind "$READBACK_SAMPLE_LIMIT_PER_KIND" --verify-readbacks \
...
printf 'process_label\trepetition\tquery_label_storage\tphase\tfile_count\tresident_bytes\tcorpus_file_bytes\n' \
    >"$RESIDENCY_SUMMARY"
printf 'process_label\tquery_name\tcategory\tmode\trepetition\torder_index\tquery_label_storage\tcorpus\traw_output\tprocess_wall_seconds\tprocess_user_seconds\tprocess_system_seconds\tmax_rss_kib\n' \
    >"$RAW_INDEX"
...
run_process() {
    local query_name="$1" category="$2" mode="$3" start_ms="$4" end_ms="$5"
    local step_ms="$6" cache_bytes="$7" boundaries_csv="$8" expression="$9"
    local repetition="${10}" order_index="${11}" policy="${12}"
    local process_label run_dir raw markdown log time_file resident_after_evict status boundary
    ...
    snapshot_residency "$process_label" "$repetition" "$policy" after-run \
...
python3 "$FROZEN_GATE_TOOL" compare-results \
    ...
    || die "semantic, QueryStats, payload, integrity, cache, or atom gate failed"

note "re-inventorying Schema 8 to prove the corpus stayed immutable"
python3 "$FROZEN_GATE_TOOL" inventory \
    --corpus "$SCHEMA8_DIR" \
