...
pub(in crate::storage::segment) fn evaluate_histogram_range_function(
    ...
    for input in series {
        ...
        let mut result = PromqlHistogramSeries::new(series_id, labels);
...
fn histogram_counter_increase(
        ...
        .all(|sample| sample.temporality == OtlpAggregationTemporality::Delta)
    {
        ...
        if non_stale_count == 1 {
            let mut increase =
                delta_histogram_interval_increase(samples, range_start_ms, range_end_ms)?;
            if let Some(range_seconds) = rate_range_seconds {
                divide_delta_histogram_rate(&mut increase, range_seconds)?;
            }
...
fn divide_delta_histogram_rate(
    increase: &mut PromqlHistogramSample,
    range_seconds: f64,
) -> Option<()> {
