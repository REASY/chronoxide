...
impl SegmentReader {
    ...
    pub(super) fn plan_generic_cross_segment_with_context(
        &self,
        context: &mut SegmentQueryContext,
        matchers: &[NormalizedMatcher],
        projection: &SegmentProjection,
        ...
        for planned in matched_entries {
            let Some(chunks) = chunk_entries_by_range.get(&planned.chunk_index) else {
                continue;
            };
            ...
            for chunk_entry in chunks.iter() {
                if chunk_entry.max_time_ms < start_ms || chunk_entry.min_time_ms > end_ms {
                    continue;
                }
                let read_len = if typed_scalar_projection(projection, chunk_entry.kind).is_some() {
    ...
    pub(super) fn decode_generic_cross_segment_plan(
        &self,
        ...
        for planned in plan.series {
            let labels = planned.labels.as_ref();
            let metric_name = labels
                .iter()
                .find_map(|(key, value)| (key == METRIC_NAME_LABEL).then_some(value.as_str()))
                .unwrap_or_default();
            ...
            for chunk_entry in planned.chunks.iter() {
                if chunk_entry.max_time_ms < start_ms || chunk_entry.min_time_ms > end_ms {
                    continue;
                }
                if let Some((scalar_projection, metric_suffix)) =
    ...
    pub(super) fn plan_native_histogram_cross_segment_with_context(
        &self,
        context: &mut SegmentQueryContext,
        matchers: &[NormalizedMatcher],
        start_ms: u64,
    ...
    pub(super) fn plan_native_exponential_histogram_cross_segment_with_context(
        &self,
        context: &mut SegmentQueryContext,
        matchers: &[NormalizedMatcher],
        start_ms: u64,
    ...
    fn plan_native_typed_cross_segment_with_context(
        &self,
        context: &mut SegmentQueryContext,
        matchers: &[NormalizedMatcher],
        projection: SegmentProjection,
    ...
    pub(super) fn decode_native_histogram_cross_segment_plan(
        &self,
    ...
    pub(super) fn decode_native_exponential_histogram_cross_segment_plan(
        &self,
        plan: NativeTypedCrossSegmentPlan,
        chunk_payloads: &ChunkPayloadBatch,
        start_ms: u64,
